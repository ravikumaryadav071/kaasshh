...
if(isset($_GET['sort_by'])){
	...
	$articles = $db_blog->query("SELECT * FROM articles ORDER BY {$sort_by} DESC", array(), 'SELECT *')->assocresults();
	$count = $db_blog->count();
			...
			if(1){
				...
				if($pages > 1){
					for($i=1; $i<=$pages; $i++){
						...
						?>
						<li>
							<a href="sort_content.php?page_no=<?php echo $i?>&sort_by=<?php echo $sort_by; ?>">
								<?php
								if(empty($_GET) && $i==1){
